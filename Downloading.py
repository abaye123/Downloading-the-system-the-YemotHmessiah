...
def process_item(item, parent_path):
    if item['fileType'] == 'DIR' or item['fileType'] == 'EXT':
        # Create a subdirectory with the same name as the item
        ...
        # Fetch all items in the subdirectory
...
paths = input(
    '\033[33mהזן נתיב הורדה מותאם אישית, בפורמט "2" או "9/1", להורדת המערכת כולה השאר ריק: \033[0m')

root = tk.Tk()
root.withdraw()
folder_path = filedialog.askdirectory()
if folder_path:
    new_folder_path = folder_path + "/" + user + "/" + paths + "/"
...
response = requests.get(
    f'https://call2all.co.il/ym/api/GetIvrTree?path=ivr2:{paths}/&token={token}')
try:
    response.raise_for_status()
except requests.exceptions.RequestException as e:
    print(f'Error fetching items: {str(e)}')
